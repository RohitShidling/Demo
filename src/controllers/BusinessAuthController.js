...
exports.register = async (req, res, next) => {
    try {
        ...
        const result = await BusinessAuthService.register({ name, email, otp });
        res.status(201).json({
            success: true,
...
exports.requestRegisterOtp = async (req, res, next) => {
    try {
        ...
        const result = await BusinessAuthService.requestRegisterOtp({ name, email });
...
exports.login = async (req, res, next) => {
    try {
        ...
        const result = await BusinessAuthService.login({ email, otp });
...
exports.requestLoginOtp = async (req, res, next) => {
    try {
        ...
        const result = await BusinessAuthService.requestLoginOtp({ email });
        res.status(200).json({ success: true, message: result.message, data: { email, expiresAt: result.expiresAt } });
    } catch (error) { next(error); }
};
