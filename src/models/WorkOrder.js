...
class WorkOrderModel {
    static async create({ work_order_id, work_order_name, target, description, created_by }) {
        const pool = getPool();
        const query = `
            INSERT INTO work_orders (work_order_id, work_order_name, target, description, created_by)
            VALUES (?, ?, ?, ?, ?)
        `;
        const [result] = await pool.execute(query, [work_order_id, work_order_name, target, description, created_by]);
