...
class BusinessUserModel {
    ...
    static async findByUsername(username) {
        const pool = getPool();
        const query = `SELECT * FROM business_users WHERE username = ?`;
    ...
    static async findByEmail(email) {
        const pool = getPool();
        const query = `SELECT * FROM business_users WHERE email = ?`;
    ...
    static async updateLastLogin(id) {
        const pool = getPool();
        const query = `UPDATE business_users SET last_login = CURRENT_TIMESTAMP(3) WHERE id = ?`;
        await pool.execute(query, [id]);
    }

    static async updateRefreshToken(id, refreshToken) {
        const pool = getPool();
        const query = `UPDATE business_users SET refresh_token = ? WHERE id = ?`;
        await pool.execute(query, [refreshToken, id]);
    }

    static async clearRefreshToken(id) {
        const pool = getPool();
        const query = `UPDATE business_users SET refresh_token = NULL WHERE id = ?`;
        await pool.execute(query, [id]);
    }

    static async findByRefreshToken(refreshToken) {
        const pool = getPool();
        const query = `SELECT * FROM business_users WHERE refresh_token = ?`;
    ...
    static async count() {
        const pool = getPool();
        const query = `SELECT COUNT(*) as count FROM business_users`;
        const [rows] = await pool.execute(query);
        return rows[0].count;
    }
}
